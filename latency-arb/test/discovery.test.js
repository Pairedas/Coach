...
test('la decouverte pagine jusqu\'a epuisement des resultats', async () => {
  ...
  assert.equal(pages.length, 3, 'trois appels attendus, puis arret sur page incomplete');
  assert.ok(pages[1].includes('offset=100'));
...
test('les marches hors fenetre d\'echeance sont ecartes', async () => {
  const getJson = async (url) => (Number(new URL(url).searchParams.get('offset')) === 0
    ? [
...
test('le seuil d\'un marche up/down est reconstruit depuis l\'ouverture horaire', async () => {
  const getJson = async (url) => (Number(new URL(url).searchParams.get('offset')) === 0
    ? [marche(1, { question: 'Bitcoin Up or Down - August 16, 1PM ET' })]
    : []);
  let demande = null;
